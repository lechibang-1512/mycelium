...
module.exports = () => {
  ...
  // Validate numeric route parameters early to avoid invalid DB queries
  router.param('id', (req, res, next, id) => {
    const n = parseInt(id, 10);
    if (Number.isNaN(n)) return res.status(400).json({ success: false, error: 'Invalid supplier id' });
    req.params.id = n;
    next();
  });
  ...
  router.get('/:supplierId/products/:productId/compatibility', async (req, res) => {
    try {
  ...
  router.get('/:id/products', async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        products: products.map(p => convertBigIntToNumber(p)),
