...
module.exports = (_pool) => {
  ...
  router.get('/lockdown-status', asyncHandler(async (req, res) => {
  ...
  router.get('/due-items', asyncHandler(async (req, res) => {
  ...
  router.put('/:id/status', asyncHandler(async (req, res) => {
    ...
    const stocktake = await StocktakeService.updateStatus(stocktake_id, status, 1, notes);
    res.json({ success: true, message: `Stocktake status updated to ${status}`, data: stocktake });
  }));
  ...
  router.put('/items/:item_id', asyncHandler(async (req, res) => {
  ...
  router.put('/items/:item_id/count', asyncHandler(async (req, res) => {
    const item_id = parseIdParam(req.params.item_id, 'item_id');
    if (!item_id) {
      return res.status(400).json({ success: false, error: 'Invalid item ID' });
    }
  ...
  router.delete('/items/:item_id', asyncHandler(async (req, res) => {
    const item_id = parseIdParam(req.params.item_id, 'item_id');
    if (!item_id) {
      return res.status(400).json({ success: false, error: 'Invalid item ID' });
    }
