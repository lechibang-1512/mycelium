...
module.exports = () => {
    ...
    const { PERMISSIONS } = require('../../shared/permissions.cjs');
     ...
     */
    router.post('/inventory-movement/warehouse-transfer', requirePermission(PERMISSIONS.INVENTORY_WRITE), async (req, res) => {
        try {
    ...
    /**
     * POST /api/inventory-movement/bin-transfer
     * Transfer inventory between bins
     */
    router.post('/inventory-movement/bin-transfer', requirePermission(PERMISSIONS.INVENTORY_WRITE), async (req, res) => {
        try {
            const result = await inventoryService.binTransfer(req.body);

            sendSuccess(res, { data: result }, result.isCrossWarehouse
                ? `Successfully moved ${result.quantity} units between warehouses`
                : `Successfully moved ${result.quantity} units between bins`);
        } catch (error) {
            console.error('Bin transfer error:', error);
            if (error.message.includes('Missing') || error.message.includes('must be different')) {
                return sendError(res, error.message, 400);
            }
            sendError(res, error.message || 'Failed to transfer between bins', error.statusCode || 500, error.details);
        }
    });
     ...
     */
    router.get('/inventory-movement/history', async (req, res) => {
        try {
     ...
     */
    router.post('/inventory-movement/bulk-transfer', async (req, res) => {
        try {
     ...
     */
    router.get('/inventory-movement/validate', async (req, res) => {
        try {
     ...
     */
    router.post('/inventory-transactions/receive', requirePermission(PERMISSIONS.INVENTORY_WRITE), async (req, res, _next) => {
        try {
            const {
                supplier_id,
                items,
     ...
     */
    router.post('/inventory-transactions/dispense', requirePermission(PERMISSIONS.INVENTORY_WRITE), async (req, res, _next) => {
        try {
            const {
                items,
                warehouse_id,
     ...
     */
    router.post('/inventory-transactions/transfer', requirePermission(PERMISSIONS.INVENTORY_WRITE), async (req, res, _next) => {
        try {
            const {
                product_id,
                asset_id,
     ...
     */
    router.get('/inventory-transactions/inventory-level/:productId', async (req, res, _next) => {
        try {
     ...
     */
    router.get('/inventory-transactions/history/:productId', async (req, res, _next) => {
        try {
            const productId = req.params.productId;
    ...
    /**
     * POST /api/inventory-transactions/batch-receive
     * Receive multiple items in batch
     */
    router.post('/inventory-transactions/batch-receive', requirePermission(PERMISSIONS.INVENTORY_WRITE), async (req, res, _next) => {
        try {
            const result = await inventoryService.receiveStock({
                ...req.body,
                user_id: req.user?.id || null // Authentication disabled
            ...
            sendSuccess(res, {
                message: `Batch receive completed: ${result.total_items} items processed`,
                data: convertBigIntToNumber(result)
            }, null, 201);

        } catch (err) {
            console.error('❌ Batch receive error:', err);
            sendError(res, err.message || 'Failed to process batch receive');
        }
    });
     ...
     */
    router.get('/inventory-transactions/validate-availability', async (req, res, _next) => {
        try {
     ...
     */
    router.get('/inventory-transactions/stats', async (req, res, _next) => {
        try {
            const warehouseId = req.query.warehouse_id || null;
     ...
     */
    router.get('/inventory-transactions/manifest/:uuid', async (req, res) => {
        try {
