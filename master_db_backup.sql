...
(1,NULL,'DATABASE_MIGRATION','ABC_ANALYSIS',NULL,NULL,NULL,NULL,NULL,'Added ABC analysis tables and views for Pareto classification','2025-12-10 11:08:27'),
(2,NULL,'DATABASE_MIGRATION','ABC_ANALYSIS',NULL,NULL,NULL,NULL,NULL,'Added ABC analysis tables and views for Pareto classification','2025-12-10 11:30:04');
...
(1,4,430,NULL,NULL,12,'2026-01-08 18:12:13','2026-01-08 20:41:42');
...
(4,652,'B001','standard',1000,1,'','2026-01-08 18:12:02','2026-01-08 20:54:15',NULL,NULL,NULL,0,NULL,NULL,NULL,NULL,NULL,NULL,'normal','easy',NULL,NULL,NULL,NULL);
...
CREATE TABLE `cycle_count_schedules` (
  ...
  `frequency` enum('daily','weekly','bi_weekly','monthly','quarterly') DEFAULT 'monthly',
  ...
  CONSTRAINT `fk_ccs_warehouse` FOREIGN KEY (`warehouse_id`) REFERENCES `warehouses` (`warehouse_id`) ON DELETE SET NULL
...
CREATE TABLE `email_notification_settings` (
  ...
  `event_type` enum('JOB_CREATED','STATUS_CHANGED','TECHNICIAN_ASSIGNED','JOB_COMPLETED','JOB_OVERDUE','PARTS_ORDERED') NOT NULL,
...
(1,'JOB_CREATED',1,0,0,NULL,'Repair Job Created - {{job_number}}','Dear {{customer_name}},\n\nYour repair job has been created.\n\nJob Number: {{job_number}}\nDevice: {{device_name}}\nIssue: {{issue_description}}\nEstimated Cost: ${{estimated_cost}}\n\nWe will keep you updated on the progress.\n\nThank you!',1,'2025-12-27 17:57:24','2025-12-27 17:57:24'),
(2,'STATUS_CHANGED',1,0,0,NULL,'Repair Job Status Update - {{job_number}}','Dear {{customer_name}},\n\nYour repair job status has been updated.\n\nJob Number: {{job_number}}\nNew Status: {{new_status}}\nDevice: {{device_name}}\n\nThank you!',1,'2025-12-27 17:57:24','2025-12-27 17:57:24'),
(3,'TECHNICIAN_ASSIGNED',0,1,0,NULL,'New Repair Job Assigned - {{job_number}}','Hello {{technician_name}},\n\nA new repair job has been assigned to you.\n\nJob Number: {{job_number}}\nDevice: {{device_name}}\nIssue: {{issue_description}}\nPriority: {{priority}}\n\nPlease review and start work.\n\nThank you!',1,'2025-12-27 17:57:24','2025-12-27 17:57:24'),
(4,'JOB_COMPLETED',1,0,0,NULL,'Repair Job Completed - {{job_number}}','Dear {{customer_name}},\n\nGreat news! Your repair job is complete and ready for pickup.\n\nJob Number: {{job_number}}\nDevice: {{device_name}}\nFinal Cost: ${{final_cost}}\nWarranty: {{warranty_months}} months\n\nPlease visit us to collect your device.\n\nThank you!',1,'2025-12-27 17:57:24','2025-12-27 17:57:24'),
(5,'JOB_OVERDUE',0,1,1,NULL,'Overdue Repair Job Alert - {{job_number}}','Alert: Repair job is overdue.\n\nJob Number: {{job_number}}\nDevice: {{device_name}}\nCustomer: {{customer_name}}\nEstimated Completion: {{estimated_completion_date}}\nDays Overdue: {{days_overdue}}\n\nPlease prioritize this job.',1,'2025-12-27 17:57:24','2025-12-27 17:57:24'),
(6,'PARTS_ORDERED',0,1,0,NULL,'Parts Ordered for Repair Job - {{job_number}}','Hello {{technician_name}},\n\nParts have been ordered for repair job {{job_number}}.\n\nDevice: {{device_name}}\nParts will arrive soon. Job status updated to PARTS_ORDERED.\n\nThank you!',1,'2025-12-27 17:57:24','2025-12-27 17:57:24');
/*!40000 ALTER TABLE `email_notification_settings` ENABLE KEYS */;
UNLOCK TABLES;
commit;

--
-- Table structure for table `expected_serials`
--

DROP TABLE IF EXISTS `expected_serials`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `expected_serials` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `invoice_id` int(11) NOT NULL,
  `product_id` varchar(36) NOT NULL,
  `expected_serial` varchar(100) DEFAULT NULL,
  `expected_imei` varchar(20) DEFAULT NULL,
  ...
  KEY `idx_product` (`product_id`),
  KEY `idx_serial` (`expected_serial`),
  KEY `idx_imei` (`expected_imei`),
  KEY `idx_received` (`is_received`),
  CONSTRAINT `fk_es_invoice` FOREIGN KEY (`invoice_id`) REFERENCES `invoices` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `expected_serials`
--

LOCK TABLES `expected_serials` WRITE;
/*!40000 ALTER TABLE `expected_serials` DISABLE KEYS */;
set autocommit=0;
/*!40000 ALTER TABLE `expected_serials` ENABLE KEYS */;
UNLOCK TABLES;
commit;
...
CREATE TABLE `inventory_log` (
  ...
  `transaction_type` enum('incoming','outgoing','transfer','rma_return','rma_disposition','zone_transfer_out','zone_transfer_in','zone_to_bin','bin_to_zone','bin_transfer','bin_deletion_return') NOT NULL,
  ...
  KEY `idx_inventory_log_outgoing` (`transaction_type`,`transaction_date`,`product_id`),
...
(1384,430,NULL,NULL,'zone_to_bin',1,'2026-01-08 20:41:42',NULL,NULL,729,652,NULL,NULL,NULL,'Moved from zone \'Z1\' to bin \'B001\'','2026-01-08 20:41:42',NULL,0.00,NULL,NULL,NULL,'2026-01-08 20:41:42',NULL,NULL,0.00,0.00,0.00,NULL,NULL),
(1494,430,NULL,NULL,'outgoing',-121,'2026-01-13 18:12:03',NULL,NULL,729,652,'OUT-20260113-00001',NULL,NULL,NULL,'2026-01-13 18:12:03',NULL,21296.00,NULL,NULL,70,'2026-01-13 18:12:03',NULL,NULL,19360.00,1936.00,21296.00,'OUT-20260113-00001',1),
(1501,NULL,NULL,NULL,'incoming',3,'2026-01-13 20:58:02',NULL,NULL,NULL,NULL,'IN-20260113-00003',1,NULL,NULL,'2026-01-13 20:58:02',100.00,310.00,NULL,NULL,3,'2026-01-13 20:58:02',NULL,NULL,300.00,10.00,310.00,'IN-20260113-00003',1),
(1502,NULL,NULL,159,'incoming',1,'2026-01-13 20:58:57',NULL,NULL,NULL,NULL,'IN-20260113-00004',1,NULL,NULL,'2026-01-13 20:58:57',100.00,103.33,NULL,NULL,NULL,'2026-01-13 20:58:57',NULL,NULL,100.00,3.33,103.33,'IN-20260113-00004',1),
(1513,NULL,NULL,170,'incoming',1,'2026-01-13 21:01:23',NULL,NULL,NULL,NULL,'IN-20260113-00007',1,NULL,NULL,'2026-01-13 21:01:23',100.00,103.33,NULL,NULL,NULL,'2026-01-13 21:01:23',NULL,NULL,100.00,3.33,103.33,'IN-20260113-00007',1),
(1527,NULL,NULL,NULL,'outgoing',-5,'2026-01-13 21:40:32',NULL,NULL,NULL,NULL,NULL,1,NULL,'Used Part #35 in Repair Job TEST-RPR-001','2026-01-13 21:40:32',NULL,NULL,NULL,NULL,NULL,'2026-01-13 21:40:32',NULL,69,0.00,0.00,0.00,NULL,NULL),
(1528,NULL,NULL,NULL,'outgoing',-5,'2026-01-13 21:41:39',NULL,NULL,NULL,NULL,NULL,1,NULL,'Used Part #37 in Repair Job TEST-RPR-001','2026-01-13 21:41:39',NULL,NULL,NULL,NULL,NULL,'2026-01-13 21:41:39',NULL,70,0.00,0.00,0.00,NULL,NULL),
(1529,NULL,NULL,NULL,'outgoing',-5,'2026-01-13 21:42:07',NULL,NULL,NULL,NULL,NULL,1,NULL,'Used Part #38 in Repair Job TEST-RPR-001','2026-01-13 21:42:07',NULL,NULL,NULL,NULL,NULL,'2026-01-13 21:42:07',NULL,71,0.00,0.00,0.00,NULL,NULL),
(1530,NULL,NULL,NULL,'outgoing',-5,'2026-01-13 21:42:35',NULL,NULL,NULL,NULL,NULL,1,NULL,'Used Part #39 in Repair Job TEST-RPR-001','2026-01-13 21:42:35',NULL,NULL,NULL,NULL,NULL,'2026-01-13 21:42:35',NULL,72,0.00,0.00,0.00,NULL,NULL),
(1531,NULL,NULL,NULL,'incoming',5,'2026-01-13 21:42:35',NULL,NULL,NULL,NULL,NULL,1,NULL,'Restored Part #39 from Repair Job TEST-RPR-001','2026-01-13 21:42:35',NULL,NULL,NULL,NULL,NULL,'2026-01-13 21:42:35',NULL,72,0.00,0.00,0.00,NULL,NULL),
(1532,NULL,NULL,NULL,'outgoing',-5,'2026-01-13 21:43:12',NULL,NULL,NULL,NULL,NULL,1,NULL,'Used Part #40 in Repair Job TEST-RPR-001','2026-01-13 21:43:12',NULL,NULL,NULL,NULL,NULL,'2026-01-13 21:43:12',NULL,73,0.00,0.00,0.00,NULL,NULL),
(1533,NULL,NULL,NULL,'incoming',5,'2026-01-13 21:43:12',NULL,NULL,NULL,NULL,NULL,1,NULL,'Restored Part #40 from Repair Job TEST-RPR-001','2026-01-13 21:43:12',NULL,NULL,NULL,NULL,NULL,'2026-01-13 21:43:12',NULL,73,0.00,0.00,0.00,NULL,NULL),
(1534,NULL,NULL,NULL,'outgoing',-5,'2026-01-14 10:18:54',NULL,NULL,NULL,NULL,NULL,1,NULL,'Used Part #41 in Repair Job TEST-RPR-001','2026-01-14 10:18:54',NULL,NULL,NULL,NULL,NULL,'2026-01-14 10:18:54',NULL,75,0.00,0.00,0.00,NULL,NULL),
(1535,NULL,NULL,NULL,'incoming',5,'2026-01-14 10:18:54',NULL,NULL,NULL,NULL,NULL,1,NULL,'Restored Part #41 from Repair Job TEST-RPR-001','2026-01-14 10:18:54',NULL,NULL,NULL,NULL,NULL,'2026-01-14 10:18:54',NULL,75,0.00,0.00,0.00,NULL,NULL),
(1596,NULL,NULL,NULL,'incoming',50,'2026-01-25 01:52:37',NULL,NULL,NULL,NULL,NULL,NULL,770,NULL,'2026-01-25 01:52:37',50.00,NULL,NULL,NULL,NULL,'2026-01-25 01:52:37',NULL,NULL,0.00,0.00,2500.00,'REC-1769280757580',NULL),
(1647,NULL,NULL,NULL,'incoming',10,'2026-01-25 02:04:37',NULL,NULL,NULL,NULL,'REC-260124-0003',1,NULL,NULL,'2026-01-25 02:04:37',100.00,1050.00,NULL,NULL,10,'2026-01-25 02:04:37',NULL,NULL,1000.00,50.00,1050.00,'REC-260124-0003',1),
(1648,NULL,NULL,NULL,'incoming',10,'2026-01-25 02:04:50',NULL,NULL,NULL,NULL,'REC-260124-0004',1,NULL,NULL,'2026-01-25 02:04:50',100.00,1050.00,NULL,NULL,10,'2026-01-25 02:04:50',NULL,NULL,1000.00,50.00,1050.00,'REC-260124-0004',1),
(1650,NULL,NULL,NULL,'incoming',10,'2026-01-25 02:05:21',NULL,NULL,NULL,NULL,'REC-260124-0006',1,NULL,'Test Receive Legacy','2026-01-25 02:05:21',100.00,1050.00,NULL,NULL,10,'2026-01-25 02:05:21',NULL,NULL,1000.00,50.00,1050.00,'REC-260124-0006',1),
(1654,NULL,NULL,NULL,'incoming',50,'2026-01-25 02:06:25',NULL,NULL,NULL,NULL,'REC-260124-0004',NULL,NULL,NULL,'2026-01-25 02:06:25',25.00,1250.00,NULL,NULL,50,'2026-01-25 02:06:25',NULL,NULL,1250.00,0.00,1250.00,'REC-260124-0004',1),
(1655,NULL,NULL,NULL,'incoming',20,'2026-01-25 02:06:25',NULL,NULL,NULL,NULL,'REC-260124-0005',NULL,NULL,NULL,'2026-01-25 02:06:25',25.00,500.00,NULL,NULL,70,'2026-01-25 02:06:25',NULL,NULL,500.00,0.00,500.00,'REC-260124-0005',1),
(1656,NULL,NULL,NULL,'incoming',50,'2026-01-25 02:08:19',NULL,NULL,NULL,NULL,'REC-260124-0006',NULL,NULL,NULL,'2026-01-25 02:08:19',25.00,1250.00,NULL,NULL,50,'2026-01-25 02:08:19',NULL,NULL,1250.00,0.00,1250.00,'REC-260124-0006',1),
(1657,NULL,NULL,NULL,'incoming',20,'2026-01-25 02:08:20',NULL,NULL,NULL,NULL,'REC-260124-0007',NULL,NULL,NULL,'2026-01-25 02:08:20',25.00,500.00,NULL,NULL,70,'2026-01-25 02:08:20',NULL,NULL,500.00,0.00,500.00,'REC-260124-0007',1),
(1671,NULL,NULL,NULL,'incoming',10,'2026-01-25 02:08:33',NULL,NULL,NULL,NULL,'REC-260124-0008',1,NULL,'Test Receive Legacy','2026-01-25 02:08:33',100.00,1050.00,NULL,NULL,10,'2026-01-25 02:08:33',NULL,NULL,1000.00,50.00,1050.00,'REC-260124-0008',1),
(1672,NULL,NULL,NULL,'incoming',5,'2026-01-25 02:08:33',NULL,NULL,NULL,NULL,'REC-260124-0009',888,NULL,'Test Receive Modern','2026-01-25 02:08:33',200.00,1020.00,NULL,NULL,5,'2026-01-25 02:08:33',NULL,NULL,1000.00,20.00,1020.00,'REC-260124-0009',1),
(1675,NULL,NULL,NULL,'incoming',10,'2026-01-25 02:08:35',NULL,NULL,NULL,NULL,NULL,NULL,NULL,'Initial inventory','2026-01-25 02:08:35',NULL,0.00,NULL,NULL,10,'2026-01-25 02:08:35',NULL,NULL,0.00,0.00,0.00,NULL,NULL),
(1679,NULL,NULL,NULL,'incoming',10,'2026-01-25 02:20:53',NULL,NULL,NULL,NULL,NULL,NULL,NULL,'Initial inventory','2026-01-25 02:20:53',NULL,0.00,NULL,NULL,10,'2026-01-25 02:20:53',NULL,NULL,0.00,0.00,0.00,NULL,NULL),
(1680,NULL,NULL,NULL,'incoming',10,'2026-01-25 02:22:01',NULL,NULL,NULL,NULL,NULL,NULL,NULL,'Initial inventory','2026-01-25 02:22:01',NULL,0.00,NULL,NULL,10,'2026-01-25 02:22:01',NULL,NULL,0.00,0.00,0.00,NULL,NULL),
(1681,NULL,NULL,NULL,'incoming',50,'2026-01-25 02:23:07',NULL,NULL,NULL,NULL,'REC-260124-0010',NULL,NULL,NULL,'2026-01-25 02:23:07',25.00,1250.00,NULL,NULL,50,'2026-01-25 02:23:07',NULL,NULL,1250.00,0.00,1250.00,'REC-260124-0010',1),
(1682,NULL,NULL,NULL,'incoming',20,'2026-01-25 02:23:07',NULL,NULL,NULL,NULL,'REC-260124-0011',NULL,NULL,NULL,'2026-01-25 02:23:07',25.00,500.00,NULL,NULL,70,'2026-01-25 02:23:07',NULL,NULL,500.00,0.00,500.00,'REC-260124-0011',1);
...
CREATE TABLE `product_inventory` (
  ...
  `condition_status` enum('NEW','GOOD','FAIR','POOR','DEFECTIVE') DEFAULT 'NEW',
  `status` enum('AVAILABLE','RESERVED','IN_TRANSIT','SOLD') DEFAULT 'AVAILABLE',
  `notes` text DEFAULT NULL,
  ...
  CONSTRAINT `fk_pi_warehouse` FOREIGN KEY (`warehouse_id`) REFERENCES `warehouses` (`warehouse_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `products` (
  `product_id` varchar(36) NOT NULL,
  `product_type` enum('PHONE','SPARE_PART','ACCESSORY','OTHER') NOT NULL DEFAULT 'PHONE',
  `name` varchar(255) NOT NULL,
  `description` text DEFAULT NULL,
  ...
  `serial_format` varchar(50) DEFAULT 'IMEI',
...
set autocommit=0;
INSERT INTO `reorder_recommendations` VALUES
(43,430,NULL,0.00,0.00,1.00,'CRITICAL',NULL,'Consider restocking','ACKNOWLEDGED',1,'2026-01-08 17:24:41','2025-12-22 11:13:05','2026-01-08 10:24:41');
...
set autocommit=0;
INSERT INTO `repair_job_parts_usage` VALUES
(35,78,52,NULL,1000,50.00,50000.00,'2026-01-25 02:08:33',NULL,NULL,NULL,'2026-01-25 02:08:33');
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`lechibang`@`localhost`*/ /*!50003 TRIGGER trg_update_repair_job_costs
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`lechibang`@`localhost`*/ /*!50003 TRIGGER trg_decrease_inventory_on_usage
AFTER INSERT ON repair_job_parts_usage
FOR EACH ROW
BEGIN
...
CREATE TABLE `repair_job_status_history` (
  ...
  `repair_job_id` int(11) NOT NULL,
  `from_status` enum('PENDING','DIAGNOSED','PARTS_ORDERED','IN_PROGRESS','TESTING','COMPLETED','DELIVERED','CANCELLED') DEFAULT NULL,
  `to_status` enum('PENDING','DIAGNOSED','PARTS_ORDERED','IN_PROGRESS','TESTING','COMPLETED','DELIVERED','CANCELLED') NOT NULL,
...
set autocommit=0;
INSERT INTO `repair_job_templates` VALUES
(1,'Screen Replacement - Standard','SCREEN_REPAIR','Standard screen replacement for most smartphone models','NORMAL',150.00,50.00,2,NULL,NULL,'Screen cracked/damaged. Display functional: Yes/No. Touch responsive: Yes/No.','Replaced screen assembly. Tested display and touch functionality. Quality check passed.',3,1,'2025-12-27 17:57:08','2025-12-27 17:57:08',NULL),
(2,'Battery Replacement - Standard','BATTERY_REPLACEMENT','Standard battery replacement service','NORMAL',80.00,30.00,1,NULL,NULL,'Battery health degraded. Current capacity: ___%. Device shuts down unexpectedly: Yes/No.','Replaced battery. Tested charging and power management. Calibrated battery.',6,1,'2025-12-27 17:57:08','2025-12-27 17:57:08',NULL),
(3,'Charging Port Repair','CHARGING_PORT','Charging port cleaning or replacement','HIGH',60.00,40.00,1,NULL,NULL,'Charging port not working. Physical damage visible: Yes/No. Debris present: Yes/No.','Cleaned/replaced charging port. Tested with multiple cables. Charging functional.',3,1,'2025-12-27 17:57:08','2025-12-27 17:57:08',NULL),
(4,'Water Damage Assessment','WATER_DAMAGE','Complete water damage diagnostic and repair','URGENT',200.00,100.00,4,NULL,NULL,'Device exposed to liquid. Liquid indicators triggered: Yes/No. Device powers on: Yes/No.','Disassembled device. Cleaned corrosion. Replaced damaged components. Tested all functions.',1,1,'2025-12-27 17:57:08','2025-12-27 17:57:08',NULL),
(5,'Software Issue - General','SOFTWARE_ISSUE','Software troubleshooting and repair','NORMAL',50.00,50.00,2,NULL,NULL,'Software malfunction. Symptoms: ___. Boot loop: Yes/No. Factory reset attempted: Yes/No.','Diagnosed software issue. Performed factory reset/firmware update. Restored functionality.',1,1,'2025-12-27 17:57:08','2025-12-27 17:57:08',NULL);
...
('RMA-000001','Bang',NULL,NULL,NULL,NULL,'defective',NULL,'completed','low',729,652,1,NULL,'2026-01-12','2026-01-13 11:52:21','2026-01-13 11:52:24','2026-01-13 11:52:35',160.00,0.00,0.00,NULL,NULL,'[{\"item_id\":1,\"product_id\":430,\"device_identifier\":\"121212\",\"device_name\":\"realme P3 Lite\",\"batch_no\":null,\"quantity_requested\":1,\"quantity_received\":0,\"quantity_approved\":0,\"unit_price\":160,\"total_price\":160,\"condition_on_receipt\":null,\"disposition\":\"repair\",\"disposition_notes\":null,\"inspection_result\":\"pending\",\"inspection_notes\":null,\"inspected_by\":null,\"inspected_at\":null,\"repair_job_id\":61,\"notes\":null,\"created_at\":\"2026-01-10T04:40:52.846Z\",\"repair_link\":{\"linked_by\":1,\"link_reason\":\"created_with_repair_job\",\"notes\":\"Linked during repair job creation\",\"linked_at\":\"2026-01-10T05:05:39.685Z\"}}]','[{\"from_status\":null,\"to_status\":\"pending\",\"changed_by\":1,\"notes\":\"RMA request created\",\"changed_at\":\"2026-01-10T04:40:52.846Z\"},{\"from_status\":\"pending\",\"to_status\":\"awaiting_receipt\",\"changed_by\":1,\"notes\":\"Status changed to awaiting_receipt\",\"changed_at\":\"2026-01-10T04:46:45.739Z\"},{\"from_status\":\"awaiting_receipt\",\"to_status\":\"received\",\"changed_by\":1,\"notes\":\"Status changed to received\",\"changed_at\":\"2026-01-13T04:52:21.223Z\"},{\"from_status\":\"received\",\"to_status\":\"inspecting\",\"changed_by\":1,\"notes\":\"Status changed to inspecting\",\"changed_at\":\"2026-01-13T04:52:24.528Z\"},{\"from_status\":\"inspecting\",\"to_status\":\"approved\",\"changed_by\":1,\"notes\":\"Status changed to approved\",\"changed_at\":\"2026-01-13T04:52:31.747Z\"},{\"from_status\":\"approved\",\"to_status\":\"completed\",\"changed_by\":1,\"notes\":\"Status changed to completed\",\"changed_at\":\"2026-01-13T04:52:35.176Z\"}]','[]','2026-01-10 11:40:52','2026-01-13 11:52:35'),
('RMA-000002','Bang',NULL,NULL,NULL,NULL,'defective',NULL,'awaiting_receipt','medium',863,772,1,NULL,NULL,NULL,NULL,NULL,160.00,0.00,0.00,NULL,NULL,'[{\"item_id\":1,\"product_id\":430,\"device_identifier\":null,\"device_name\":\"realme P3 Lite\",\"batch_no\":null,\"quantity_requested\":1,\"quantity_received\":0,\"quantity_approved\":0,\"unit_price\":160,\"total_price\":160,\"condition_on_receipt\":null,\"disposition\":\"repair\",\"disposition_notes\":null,\"inspection_result\":\"pending\",\"inspection_notes\":null,\"inspected_by\":null,\"inspected_at\":null,\"repair_job_id\":76,\"notes\":null,\"created_at\":\"2026-01-14T03:16:35.144Z\",\"repair_link\":{\"linked_by\":1,\"link_reason\":\"created_with_repair_job\",\"notes\":\"Linked during repair job creation\",\"linked_at\":\"2026-01-14T03:28:24.572Z\"}}]','[{\"from_status\":null,\"to_status\":\"pending\",\"changed_by\":1,\"notes\":\"RMA request created\",\"changed_at\":\"2026-01-14T03:16:35.144Z\"},{\"from_status\":\"pending\",\"to_status\":\"awaiting_receipt\",\"changed_by\":1,\"notes\":\"Status changed to awaiting_receipt\",\"changed_at\":\"2026-01-14T03:28:01.890Z\"}]','[]','2026-01-14 10:16:35','2026-01-14 10:28:24');
...
set autocommit=0;
INSERT INTO `smartphone_repair_jobs` VALUES
(76,'RPR-2026-00001',430,NULL,'realme P3 Lite',NULL,'Bang',NULL,NULL,NULL,'Code: defective\n\nItem Details:',NULL,NULL,'PENDING','NORMAL','Nasm',729,NULL,'2026-01-14 10:28:24','2026-01-14 00:00:00',NULL,NULL,NULL,0.00,100.00,0.00,100.00,0.00,'USD',NULL,NULL,NULL,3,NULL,'2026-01-14 10:28:24','2026-01-14 10:31:48',NULL),
(78,'TEST-RPR-001',NULL,NULL,NULL,NULL,'Test Customer',NULL,NULL,NULL,'Broken Screen',NULL,NULL,'IN_PROGRESS','NORMAL',NULL,NULL,NULL,'2026-01-25 02:08:33',NULL,NULL,NULL,0.00,0.00,50250.00,0.00,50250.00,0.00,'USD',NULL,NULL,NULL,3,NULL,'2026-01-25 02:08:33','2026-01-25 02:08:33',NULL);
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`lechibang`@`localhost`*/ /*!50003 TRIGGER trg_log_repair_job_status_change 
AFTER UPDATE ON smartphone_repair_jobs 
FOR EACH ROW 
INSERT INTO repair_job_status_history (repair_job_id, from_status, to_status, changed_by, changed_at) 
SELECT NEW.repair_job_id, OLD.status, NEW.status, NEW.created_by, CURRENT_TIMESTAMP 
...
CREATE TABLE `smartphone_spare_parts` (
  ...
  `spare_part_uuid` varchar(36) DEFAULT NULL,
  ...
  UNIQUE KEY `idx_spare_part_uuid` (`spare_part_uuid`),
...
set autocommit=0;
INSERT INTO `smartphone_spare_parts` VALUES
(9,'a2f4f997-ff61-11f0-8fcf-da2d521265c5','MOBO-REALMEP3LITE','mobo','MOTHERBOARD',NULL,'',430,'',NULL,NULL,NULL,NULL,NULL,'STANDARD',3,'realme','',100.00,100.00,'USD',NULL,NULL,5,50,10,20,1,0,0,NULL,'2025-12-02 08:44:16','2026-02-01 18:32:06',NULL),
(10,'a2f4fae4-ff61-11f0-8fcf-da2d521265c5','LCD-1765946957043-ZGOD8K','Test LCD Screen 1765946957043','DISPLAY',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'OEM',3,'Test Manufacturer',NULL,50.00,0.00,'USD',NULL,NULL,15,50,20,20,0,0,0,NULL,'2025-12-17 11:49:17','2026-02-01 18:32:06',NULL),
(11,'a2f4fb30-ff61-11f0-8fcf-da2d521265c5','LCD-1765947932747-KGNLWE','Test LCD Screen 1765947932747','DISPLAY',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'OEM',3,'Test Manufacturer',NULL,50.00,0.00,'USD',NULL,NULL,15,50,20,20,0,0,0,NULL,'2025-12-17 12:05:32','2026-02-01 18:32:06',NULL),
(12,'a2f4fb61-ff61-11f0-8fcf-da2d521265c5','LCD-1765949097210-4BCPG8','Test LCD Screen 1765949097210','DISPLAY',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'OEM',3,'Test Manufacturer',NULL,50.00,0.00,'USD',NULL,NULL,15,50,20,20,0,0,0,NULL,'2025-12-17 12:24:57','2026-02-01 18:32:06',NULL),
(13,'a2f4fb8e-ff61-11f0-8fcf-da2d521265c5','LCD-1765949432460-WVRFNL','Test LCD Screen 1765949432460','DISPLAY',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'OEM',3,'Test Manufacturer',NULL,50.00,0.00,'USD',NULL,NULL,15,50,20,20,0,0,0,NULL,'2025-12-17 12:30:32','2026-02-01 18:32:06',NULL),
(14,'a2f4fbb8-ff61-11f0-8fcf-da2d521265c5','LCD-1766055258620-YFEVEQ','Test LCD Screen 1766055258620','DISPLAY',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'OEM',3,'Test Manufacturer',NULL,50.00,0.00,'USD',NULL,NULL,15,50,20,20,0,0,0,NULL,'2025-12-18 17:54:18','2026-02-01 18:32:06',NULL),
(15,'a2f4fbe1-ff61-11f0-8fcf-da2d521265c5','LCD-1766056585943-RK4P0F','Test LCD Screen 1766056585943','DISPLAY',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'OEM',3,'Test Manufacturer',NULL,50.00,0.00,'USD',NULL,NULL,15,50,20,20,0,0,0,NULL,'2025-12-18 18:16:25','2026-02-01 18:32:06',NULL),
(16,'a2f4fc0e-ff61-11f0-8fcf-da2d521265c5','SCREEN-REALMEP3LITE','Touchscreen','DISPLAY',NULL,'',430,NULL,NULL,NULL,NULL,NULL,NULL,'STANDARD',3,'','',12.00,12.00,'USD',NULL,NULL,5,50,10,20,0,0,0,NULL,'2025-12-27 11:43:36','2026-02-01 18:32:06',NULL),
(18,'a2f4fc41-ff61-11f0-8fcf-da2d521265c5','TEST-PART-20-ced79c45','Test Part 20','DISPLAY',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'STANDARD',3,NULL,NULL,20.00,0.00,'USD',NULL,NULL,5,50,10,20,0,0,0,NULL,'2025-12-27 22:40:51','2026-02-01 18:32:06',NULL),
(19,'a2f4fc70-ff61-11f0-8fcf-da2d521265c5','TEST-PART-20-d4627c84','Test Part 20','DISPLAY',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'STANDARD',3,NULL,NULL,20.00,0.00,'USD',NULL,NULL,5,50,10,20,0,0,0,NULL,'2025-12-27 22:41:00','2026-02-01 18:32:06',NULL),
(20,'a2f4fc9b-ff61-11f0-8fcf-da2d521265c5','TEST-PART-20-e4d54f90','Test Part 20','DISPLAY',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'STANDARD',3,NULL,NULL,20.00,0.00,'USD',NULL,NULL,5,50,10,20,0,0,0,NULL,'2025-12-27 22:41:27','2026-02-01 18:32:06',NULL),
(52,'a2f4fcc5-ff61-11f0-8fcf-da2d521265c5','PART-CONS-001','Test Part Consumption','DISPLAY',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'STANDARD',3,NULL,NULL,50.00,0.00,'USD',NULL,NULL,5,50,10,20,1,0,0,NULL,'2026-01-25 02:08:33','2026-02-01 18:32:06',NULL),
(54,'a2f4fcf0-ff61-11f0-8fcf-da2d521265c5','LCD-1769281714350-JYKXH3','Test LCD Screen 1769281714350','DISPLAY',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'STANDARD',3,NULL,NULL,0.00,0.00,'USD',NULL,NULL,5,50,10,20,1,0,0,NULL,'2026-01-25 02:08:34','2026-02-01 18:32:06',NULL);
...
set autocommit=0;
INSERT INTO `smartphone_spare_parts_inventory` VALUES
(15,9,729,652,NULL,10,0,0,0,NULL,NULL,NULL,NULL,'NEW',NULL,NULL,NULL,NULL,'2026-01-14 10:45:24','ISSUE','2026-01-08 21:00:31','2026-01-14 10:45:24'),
(58,52,1240,NULL,NULL,100,0,0,0,NULL,NULL,NULL,NULL,'NEW',NULL,NULL,NULL,NULL,NULL,NULL,'2026-01-25 02:08:33','2026-01-25 02:08:33');
...
CREATE TABLE `spare_parts_reorder_recommendations` (
  ...
  CONSTRAINT `spare_parts_reorder_recommendations_ibfk_2` FOREIGN KEY (`warehouse_id`) REFERENCES `warehouses` (`warehouse_id`) ON DELETE SET NULL
...
set autocommit=0;
INSERT INTO `spare_parts_reorder_recommendations` VALUES
(6,9,NULL,0,10,20,'CRITICAL',NULL,'Consider restocking','ORDERED',NULL,'2025-12-22 10:39:55','2025-12-27 04:43:48'),
(7,16,NULL,0,10,20,'CRITICAL',NULL,'Consider restocking','CANCELLED',11,'2025-12-27 04:43:45','2026-01-13 03:19:14'),
(8,9,NULL,0,10,20,'CRITICAL',NULL,'Consider restocking','CANCELLED',11,'2025-12-27 04:43:54','2026-01-13 03:19:13'),
(9,18,NULL,0,10,20,'CRITICAL',NULL,'Consider restocking','CANCELLED',11,'2026-01-06 08:40:30','2026-01-13 03:19:11'),
(10,19,NULL,0,10,20,'CRITICAL',NULL,'Consider restocking','CANCELLED',11,'2026-01-06 08:40:30','2026-01-13 03:19:12'),
(11,20,NULL,0,10,20,'CRITICAL',NULL,'Consider restocking','CANCELLED',11,'2026-01-06 08:40:30','2026-01-13 03:19:13');
...
set autocommit=0;
INSERT INTO `specs_db` VALUES
(430,'P3 Lite','realme',160.00,'Pine Green','none','Mediatek Dimensity 6300',NULL,NULL,NULL,'Mali-G57 MC2',NULL,'6','128','Yes',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'6300','45W','USB Type-C 2.0',NULL,NULL,'No',NULL,NULL,NULL,NULL,NULL,NULL,NULL,'Android 15, Realme UI 6.0',NULL,'phone',NULL,11,0,0,7,0,0.00,NULL,NULL,NULL,NULL,NULL,NULL,NULL,12,'MANUFACTURER',NULL,1,0,NULL,NULL),
(647,'Test Phone Price','Test Maker',100.00,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,12,0,0,7,0,0.00,NULL,NULL,NULL,NULL,NULL,NULL,NULL,12,'MANUFACTURER',NULL,1,0,NULL,NULL);
...
(65,41,430,NULL,70.00,76.00,6.00,8.57,0,NULL,'Days since count: 9510','2026-01-14 10:24:14',1,'2026-01-14 03:23:56','2026-01-14 03:24:14');
...
set autocommit=0;
INSERT INTO `stocktake_status_history` VALUES
(82,41,NULL,'PLANNED',1,'2026-01-14 03:23:56','Stocktake created'),
(83,41,'PLANNED','IN_PROGRESS',1,'2026-01-14 03:24:15','Stocktake started'),
(84,41,'IN_PROGRESS','COMPLETED',1,'2026-01-14 03:24:17','Stocktake completed'),
(85,41,'COMPLETED','APPROVED',1,'2026-01-14 03:24:19','Stocktake approved'),
(86,42,NULL,'PLANNED',1,'2026-01-24 19:08:34','Stocktake created');
...
(41,'ST-20260114-O5TMZ2',729,NULL,'full','APPROVED',1,'2026-01-14 10:24:15','2026-01-14 10:24:17',1,'2026-01-14 10:24:19',NULL,NULL,0,NULL,'2026-01-14 03:23:56','2026-01-14 03:24:19'),
(42,'ST-20260124-B60HFC',1252,NULL,'full','PLANNED',1,NULL,NULL,NULL,NULL,'Test stocktake',NULL,0,NULL,'2026-01-24 19:08:34','2026-01-24 19:08:34');
...
(478,'DigiWorld','Electronics','Sam','Salesperson',NULL,NULL,NULL,NULL,NULL,1,'2025-12-15 12:49:51','2025-12-15 12:50:40',NULL,NULL,NULL,NULL),
(586,'Test Supplier','electronics',NULL,NULL,NULL,NULL,NULL,NULL,NULL,1,'2026-01-06 08:40:30','2026-01-06 08:40:30',NULL,NULL,NULL,NULL),
(589,'Test Supplier 1767688831282','electronics','John Doe',NULL,'test1767688831282@example.com','1234567890',NULL,NULL,NULL,1,'2026-01-06 08:40:31','2026-01-06 08:40:31',NULL,NULL,NULL,NULL),
(600,'Test Supplier','electronics',NULL,NULL,NULL,NULL,NULL,NULL,NULL,1,'2026-01-08 07:11:17','2026-01-08 07:11:17',NULL,NULL,NULL,NULL),
(601,'Test Supplier','electronics',NULL,NULL,NULL,NULL,NULL,NULL,NULL,1,'2026-01-08 07:12:44','2026-01-08 07:12:44',NULL,NULL,NULL,NULL),
(606,'Test Supplier','electronics',NULL,NULL,NULL,NULL,NULL,NULL,NULL,1,'2026-01-10 06:09:04','2026-01-10 06:09:04',NULL,NULL,NULL,NULL),
(629,'Test Supplier','electronics',NULL,NULL,NULL,NULL,NULL,NULL,NULL,1,'2026-01-10 06:09:25','2026-01-10 06:09:25',NULL,NULL,NULL,NULL),
(630,'Test Supplier','electronics',NULL,NULL,NULL,NULL,NULL,NULL,NULL,1,'2026-01-12 03:57:07','2026-01-12 03:57:07',NULL,NULL,NULL,NULL),
(813,'Test Supplier','electronics',NULL,NULL,NULL,NULL,NULL,NULL,NULL,1,'2026-01-24 19:08:34','2026-01-24 19:08:34',NULL,NULL,NULL,NULL),
(816,'Test Supplier 1769281714596','electronics','John Doe',NULL,'test1769281714596@example.com','1234567890',NULL,NULL,NULL,1,'2026-01-24 19:08:34','2026-01-24 19:08:34',NULL,NULL,NULL,NULL);
...
set autocommit=0;
INSERT INTO `warehouse_product_locations` VALUES
(513,729,652,430,70,0,0,'2025-12-24 10:18:55','2026-01-13 18:12:03');
...
(652,729,'Z1',NULL,'receiving',NULL,1,'2025-12-22 00:27:18','2026-01-08 20:54:01','standard','RCV',NULL,'grid',0),
(772,863,'Test Zone',NULL,'storage',NULL,1,'2026-01-10 13:09:25','2026-01-10 13:09:25','standard',NULL,NULL,'grid',0),
(773,865,'Test Zone',NULL,'storage',NULL,1,'2026-01-12 10:57:07','2026-01-12 10:57:07','standard',NULL,NULL,'grid',0),
(1038,1154,'Test Zone Price',NULL,'storage',NULL,1,'2026-01-17 11:30:23','2026-01-17 11:30:23','standard',NULL,NULL,'grid',0),
(1127,1252,'Test Zone',NULL,'storage',NULL,1,'2026-01-25 02:08:34','2026-01-25 02:08:34','standard',NULL,NULL,'grid',0),
(1128,1253,'Test Zone 2',NULL,'storage',NULL,1,'2026-01-25 02:08:34','2026-01-25 02:08:34','standard',NULL,NULL,'grid',0);
...
(729,'WH1','Homebase',NULL,NULL,1,'2025-12-22 00:26:58','2025-12-22 00:26:58'),
(863,'Test Warehouse','Test Location',NULL,NULL,1,'2026-01-10 13:09:25','2026-01-10 13:09:25'),
(865,'Test Warehouse 2','Test Location','','{\"manager_name\":\"\",\"contact_phone\":\"\",\"contact_email\":\"\"}',1,'2026-01-12 10:57:07','2026-01-13 01:49:27'),
(1154,'Test Warehouse Price','Test Location Price',NULL,NULL,1,'2026-01-17 11:30:23','2026-01-17 11:30:23'),
(1240,'Test Repair WH','Test Loc',NULL,NULL,1,'2026-01-25 02:08:33','2026-01-25 02:08:33'),
(1252,'Test Warehouse','Test Location',NULL,NULL,1,'2026-01-25 02:08:34','2026-01-25 02:08:34'),
(1253,'Test Warehouse 2','Test Location 2',NULL,NULL,1,'2026-01-25 02:08:34','2026-01-25 02:08:34');
